...
# Email IDs
receiver_email = ['chetantiwaridgp5@gmail.com', 'sainath@primetrade.ai']
...
traders_list = pd.read_csv("G:\Projects\ML\Company\debug_new\Top_traders_gmxv2_rank_15-11-2024.csv")
        ...
        def openTrade(client, traders_list, address, symbol, position_side, asset_price, transaction_amount, transaction_quantity, new_leads_leverage):
            ...
            for i in range(number_of_try):
                try:
                    ...
                    market_orderId = str(market_order['orderId'])     
                    marketOrderPrice = float(market_order.get('avgPrice', 0))
                    marketOrderQty = float(market_order.get('executedQty', 0))
                                                                                                                  ##################
                    logging.info(f"Order placed - Symbol: {symbol}, Side: {open_side}, Quantity: {marketOrderQty}, "f"Price: {marketOrderPrice}, Leverage: {our_leverage}, Address: {address}")
                    ...
                    print("Error placing order:", e)
                                                                                                                   #########################
                    logging.error(f"Error placing order on Binance - Symbol: {symbol}, Side: {open_side}, "f"Quantity: {transaction_quantity}, Price: {marketOrderPrice or 'N/A'}, "f"Error: {traceback.format_exc()}, Address: {address}")
                ...
                try:
                    market_order = client.futures_create_order(symbol=symbol, positionSide=position_side, side=close_side,
                                            type=client.FUTURE_ORDER_TYPE_MARKET, quantity=quantity_to_close)
                                                                                                           #########################
                    logging.info(f"Order placed - Symbol: {symbol}, Side: {open_side}, Quantity: {transaction_quantity}, "f"Price: {marketOrderPrice}, Leverage: {our_leverage}, Address: {address}")
                    ...
                    print("Error placing order:", e)
                                                                                                           ############################
                    logging.error(f"Error placing order on Binance - Symbol: {symbol}, Side: {open_side}, "f"Error: {traceback.format_exc()}, Address: {address}")
            ...
            for i in range(number_of_try):
                try:
                    market_order = client.futures_create_order(symbol=symbol, positionSide=position_side, side=close_side,
                                            type=client.FUTURE_ORDER_TYPE_MARKET, quantity=quantity_to_close)
                                                                                                                      ########################
                    logging.info(f"Order placed - Symbol: {symbol}, Side: {close_side}, Quantity: {quantity_to_close}, "f"Leverage: {our_leverage}, Address: {address}")
                    ...
                    print("Error placing order:", e)
                                                                                                                     ######################
                    logging.error(f"Error placing order on Binance - Symbol: {symbol}, Side: {close_side}, "f"Error: {traceback.format_exc()}, Address: {address}")
                    if i <= (number_of_try - 2):
                        print(f"Retrying placing market order in {retry_time} seconds...({i})")
                        time.sleep(retry_time)
                        continue
...
markets = {
    ...
    '0x47c031236e19d024b42f8ae6780e44a573170703': ['BTC/USD', 'BTC-USDC', 8, 22, 8, 6],
    '0x70d95587d40a2caf56bd97485ab3eec10bee6336': ['ETH/USD', 'WETH-USDC', 18, 12, 18, 6],
    '0x6853ea96ff216fab11d2d930ce3c508556a4bdc4': ['DOGE/USD', 'WETH-USDC', 8, 22, 18, 6],
    '0x09400d9db990d5ed3f35d7be61dfaeb900af03c9': ['SOL/USD', 'SOL-USDC', 9, 21, 9, 6],
    '0xd9535bb5f58a1a75032416f2dfe7880c30575a41': ['LTC/USD', 'WETH-USDC', 8, 22, 18, 6],
    '0xc7abb2c5f3bf3ceb389df0eecd6120d451170b50': ['UNI/USD', 'UNI-USDC', 18, 12, 18, 6],
    '0x7f1fa204bb700853d36994da19f830b6ad18455c': ['LINK/USD', 'LINK-USDC', 18, 12, 18, 6],
    '0xc25cef6061cf5de5eb761b50e4743c1f5d7e5407': ['ARB/USD', 'ARB-USDC', 18, 12, 18, 6],
    ...
    '0x0ccb4faa6f1f1b30911619f1184082ab4e25813c': ['XRP/USD', 'WETH-USDC', 6, 24, 18, 6],
    '0x2d340912aa47e33c90efb078e69e70efe2b34b9b': ['BNB/USD', 'BNB-USDC', 18, 12, 18, 6],
    '0x1cbba6346f110c8a5ea739ef2d1eb182990e4eb2': ['AAVE/USD', 'AAVE-USDC', 18, 12, 18, 6],
    '0x248c35760068ce009a13076d573ed3497a47bcd4': ['ATOM/USD', 'WETH-USDC', 6, 24, 18, 6],
    '0x63dc80ee90f26363b3fcd609007cc9e14c8991be': ['NEAR/USD', 'WETH-USDC', 24, 6, 18, 6],
    '0x7bbbf946883a5701350007320f525c5379b8178a': ['AVAX/USD', 'AVAX-USDC', 18, 12, 18, 6],
    '0x4fdd333ff9ca409df583f306b6f5a7ffde790739': ['OP/USD', 'OP-USDC', 18, 12, 18, 6],
    '0x7c11f78ce78768518d743e81fdfa2f860c6b9a77': ['BTC/USD', 'BTC', 8, 22, 8, 8],
    '0x450bb6774dd8a756274e0ab4107953259d2ac541': ['ETH/USD', 'WETH', 18, 12, 18, 18],
    '0x55391d178ce46e7ac8eaaea50a72d1a5a8a622da': ['GMX/USD', 'GMX-USDC', 18, 12, 18, 6],
...
class livePositionExtractiontest: # Runs the loop function to return information on a trader, if any of the 3 events are run.
    ...
    def __init__(self, proxy_contract, client, traders_list):
        ...
        # Provide your Alchemy URL or any other Ethereum node URL      
        self.w3 = Web3(Web3.HTTPProvider(alchemy_url))
        print(self.w3.is_connected())

        # Define contract objects
        ...
        self.pair_address = Web3.to_checksum_address(pair_proxy_address)
        self.pair_contract = self.w3.eth.contract(address=self.pair_address, abi=ABIs)
        
        data_address = "0xFD70de6b91282D8017aA4E741e9Ae325CAb992d8"
        self.data_store_address = Web3.to_checksum_address(data_address)

        # Get the event ABI for EventLog1 and calculate the event signature manually
        event_abi = self.contract.events.EventLog1._get_event_abi()
        
        # Use keccak to calculate the event signature
        event_signature = self.w3.keccak(text=str(event_abi)).hex()

        # Use web3.eth.filter() to create a filter
        event_filter = self.w3.eth.filter({
            'fromBlock': 'latest',
            'address': self.proxy_address,
            'topics': [event_signature]  # Use the event signature
        })
        
        # The get_new_entries method is available on the EventFilter object returned by the filter
        self.market_events = event_filter.get_new_entries

        # ----Calling the main trading things class for trading assistance----
    ...
    def loop(self):
        ...
        while True:
            logging.basicConfig(filename='alchemy_event_log.txt', level=logging.INFO, format='%(asctime)s - %(message)s') ####################

            # Add logging before processing Alchemy events
            logging.info("Starting to fetch Alchemy events...")                                ###############################################
            for event in self.market_events(): # Make sure to call it
                
                market_pair = None
                event_name = event['args']['eventName'].lower()

                if any(keyword in event_name for keyword in ['positionincrease', 'positiondecrease']):

                    address_items = {item['key']: item['value'] for item in event["args"]["eventData"]["addressItems"]["items"]}
                    uint_items = {item['key']: item['value'] for item in event["args"]["eventData"]['uintItems']['items']}
                    bool_items = {item['key']: item['value'] for item in event["args"]["eventData"]['boolItems']['items']}
                    int_items = {item['key']: item['value'] for item in event["args"]["eventData"]['intItems']['items']}
                    address = address_items['account'].lower()

                    if address in self.traders_list['account'].values:
                        logging.info(f"Event matched top trader: {address}")       #################################################

                        # added by me new
                        # Market token pair value
                        market_value = address_items["market"].lower()
                        collateral_value = address_items["collateralToken"].lower()
                        ...
                        if market_value in markets:
                            ...
                            coin_pair = market[0]
                            coin_pair2 = market[1]
                            print(f"coin_pair: {coin_pair}\n(coin_pair2: {coin_pair2})")
                            coin = coin_pair.split('/')[0]
                        else:
                            coin_pair = market_value
                            print("\nUNKNOWN Market Token (coin_pair) : ", coin_pair, "\n\n")
                            continue
                        ...
                        if collateral_value in collaterals:
                            ...
                            print("\nUNKNOWN Collateral Token : ", collateral_value, "-But continuing to TRADE with unknown collateral token-")
                            

                        execution_price = uint_items['executionPrice']
                        size_in_usd = (uint_items['sizeInUsd']) / 1e30
                        size_in_tokens = uint_items['sizeInTokens'] / 10**(market[2])
                        asset_price = execution_price / 10**(market[3])

                        if collateral_value == "USDC":
                            collateral_amount = ((uint_items['collateralAmount']) / 10**(collateral[1])) 
                        else: 
                            collateral_amount = (uint_items['collateralAmount'] * ((uint_items['collateralTokenPrice.max'] + uint_items['collateralTokenPrice.min']) / 2)) / 10**30

                        transaction_amount = uint_items['sizeDeltaUsd']/ 1e30 
                        transaction_quantity = uint_items['sizeDeltaInTokens'] / 10**(market[2])

                        print(f"size_in_usd: {size_in_usd} \nsize_in_tokens: {size_in_tokens}\ncollateral_amount: {collateral_amount}")

                        if size_in_tokens == 0 or collateral_amount == 0 or size_in_usd == 0:
                            if size_in_tokens == 0 and collateral_amount == 0 and size_in_usd == 0 and event_name == "positiondecrease":
                                ...
                                collateralDeltaAmount = (uint_items['collateralDeltaAmount'] * ((uint_items['collateralTokenPrice.max'] + uint_items['collateralTokenPrice.min']) / 2)) / 10**30
                                print("collateralDeltaAmount: ", collateralDeltaAmount)
                                new_leads_leverage = round((transaction_amount/collateralDeltaAmount), 1)
                         ...
                         # Log calculated trade parameters                 #########################################################
                        if 'execution_price' in locals() and 'size_in_usd' in locals() and 'size_in_tokens' in locals():
                            logging.info(f"Calculated Trade Parameters - Execution Price: {execution_price}, "f"Size (USD): {size_in_usd}, Size (Tokens): {size_in_tokens}, "f"Leverage: {new_leads_leverage}, Address: {address}, Event: {event_name}")
                        else:
                            logging.error("One or more trade parameters are not defined. Skipping logging for this event.") ###################


                        position_side = "LONG" if bool_items['isLong'] else "SHORT"
                        event = "OPEN" if event_name == "positionincrease" else "CLOSE"
                        order_type_value = uint_items['orderType']
                        orderType =  order_type_map.get(order_type_value, "UNKNOWN")

                        if orderType == "UNKNOWN":
                            print(f"UNKOWN orderType={order_type_value} appeared. Rejecting trade.")
                            print("event : ", event)
                            continue

                        try:
                            # stop_signal_path = "mux_stop_signal.txt"       check and delete
                        ...
                        if event_name == "positionincrease":
                            if stop_signal != "STOP":
                                ...
                                print("leads_leverage: ", new_leads_leverage)
                                print(f"transaction_amount: {transaction_amount}")
                                print(f"transaction_quantity:  {transaction_quantity}")
                        ...
                        elif event_name == "positiondecrease" and order_type_value != 7:
                            ...
                            print("transaction_quantity: ", transaction_quantity)
                            print("base_PNL: ", int_items["basePnlUsd"]/1e30 )
                            ...
                            print("position_side: ", position_side)
                            print("event : ", event)
                            print("--order_type_value--:", order_type_value)
                            print("orderType: ", orderType)
                            print("coin: ", coin)
                            print("asset_price: ", asset_price)
                            print("address: ", address)
                            ...
                            print("----UNKNOWN event_name or condition occoured in GMXV2, so rejecting the trade.----")
                            continue

                        # Log missed trades due to invalid or incomplete conditions ##############################################
                        if 'address' in locals() and 'symbol' in locals() and 'position_side' in locals():
                            logging.warning(f"Trade missed - Account: {address}, Symbol: {symbol}, Side: {position_side}, "f"Transaction Amount: {transaction_amount}, Quantity: {transaction_quantity}")
                        else:
                            logging.error("Missed trade parameters are undefined or unavailable.") ################################

                    else:
                        logging.info(f"Event skipped (not in top traders): {address}")      ########################
                        continue
